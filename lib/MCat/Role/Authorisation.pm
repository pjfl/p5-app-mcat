...
around 'allowed' => sub {
   my ($orig, $self, $context, $method) = @_;

   my $models  = $context->models;
   my $action  = $self->moniker . "/${method}";
   my $attr    = MCat::Navigation::Attributes->attr_for($models, $action);
   my $role    = $attr->{Auth}->[-1] // 'edit';

   return $orig->($self, $context, $method) if $role eq 'none';
   ...
   return $orig->($self, $context, $method) if $role eq 'view';

   my $user_role = $session->role or throw NoUserRole, [$session->username];

   return $orig->($self, $context, $method)
